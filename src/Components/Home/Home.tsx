...
const Home: React.FC = () => {
    //const [boardData, setBoardData] = useState<{ category: string; boards: string[] }[]>([]);
    ...
    useEffect(() => {
        ...
        (async () => {
            const accessToken = localStorage.getItem("jbig-accessToken");
            ...
            // api 참고 후 수정 필요
            // try {
            //     const rawData = await getBoardData();
            //
            //     // [{category, name}, ...] => [{category, boards: [...]}, ...] 변환
            //     const grouped: { category: string; boards: string[] }[] = [];
            //     rawData.forEach((item: { category: string; name: string }) => {
            //         const found = grouped.find(g => g.category === item.category);
            //         if (found) {
            //             found.boards.push(item.name);
            //         } else {
            //             grouped.push({ category: item.category, boards: [item.name] });
            //         }
            //     });
    ...
    const sidebarProps = { boardData, isAdmin, isLoggedIn, quizURL, totalCount, homeBanner, navigate };
            ...
            <header className="home-header">
                ...
                <div className="user-info">
                    {userName ? (
                        <span
                            className="user-id"
                            // onClick={() => navigate('/my')} //todo: 사용자 프로필로 이동
                        >
                            <CircleUserRound size={18} style={{ verticalAlign: "middle" }} />
                          {userName}
                        </span>
                    ) : (
                        <button
